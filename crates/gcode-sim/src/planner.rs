...
fn push_move(events: &mut Vec<MotionEvent>, state: &mut State, target: (f32, f32, f32, f32), feedrate_mm_per_min: f32) {
...
fn apply_home(events: &mut Vec<MotionEvent>, state: &mut State, axes: HomeAxes, config: &PrinterConfig) {
    let target = (
    ...
    push_move(events, state, target, config.homing_feedrate_mm_per_min);
}
    ...
    for line in gcode.lines() {
        ...
        match cmd {
            Command::RapidMove(args) | Command::LinearMove(args) => {
                ...
                push_move(&mut events, &mut state, target, feedrate);
            }
            Command::Home(axes) => apply_home(&mut events, &mut state, axes, config),
...
mod tests {
    ...
    fn wait_temp_advances_virtual_clock() {
        ...
        assert_eq!(events.last().unwrap().t, 0.0); // no motion happened
        // but the clock advanced internally; verify via a following move
